  ...
  module Controllers
    module Meine
      ...
      def admin_user_trades(data = nil)
        trads = Trade.where(buyer: sget('admin_edit_client').id, status: [Trade::FINISHED, Trade::FINALIZED])
      ...
      def show_user(data = nil)
        ...
        not_permitted if user.bot != @tsx_bot.id or (!hb_client.is_admin?(@tsx_bot) and !hb_client.is_operator?(@tsx_bot))
      ...
      def do_add_cash(data)
        ...
        cl.cashin(cents, Client::__cash, Meth::__debt, hb_client, "Оператор #{hb_client.id} пополнил счет на #{@tsx_bot.amo(cents)}")
